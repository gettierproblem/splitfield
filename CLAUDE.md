...
**Loadout System** (CursorShip + GameScene): W/scroll-up loads a laser cartridge (consumes ammo, fires 4x speed beam). D/scroll-down loads a cluster magnet (places two wall magnets that attract balls for 5s). Scrolling the opposite direction unloads (scroll down while laser loaded → unload, with 0.3s debounce before next weapon load). Space/right-click toggles H/V orientation. Touchpad pan gestures map to scroll. Ammo persists between levels.
